    ...
    <SocialBox> 
      ...
      <div className="info">
        <div className="wrap" onClick={onFacebook}><FacebookIcon style={{fontSize:25, color:'rgba(0,0,0,.8)'}}/>
          {facebook 
            ? <div className="box" onClick={onbreakEvent}>
              <Input onChange={(event)=>onChangeData(event,setFacebookData)} value={facebookData || ''} className="input" name={'주소를 입력해주세요.'} />
              <button onClick={()=>saveData(facebookData,'facebook',setFacebook)}>
                저장
              </button></div> 
            : facebookData || '정보를 입력해 주세요.'}
        </div>
        <div className="wrap" onClick={onGithub}><GitHubIcon style={{fontSize:25, color:'rgba(0,0,0,.8)'}}/>
          {github 
            ? <div className="box" onClick={onbreakEvent}>
              <Input onChange={(event)=>onChangeData(event,setGitData)} value={gitData || ''} className="input" name={'주소를 입력해주세요.'} />
              <button onClick={()=>saveData(gitData,'github',setGithub)}>
                저장
              </button></div> 
            : gitData || '정보를 입력해 주세요.'}
        </div>
        <div className="wrap" onClick={onInstagram}><InstagramIcon style={{fontSize:25, color:'rgba(0,0,0,.8)'}}/>
            {instagram 
              ? <div className="box" onClick={onbreakEvent}>
                <Input onChange={(event)=>onChangeData(event,setInstaData)} value={instaData || ''} className="input" name={'주소를 입력해주세요.'} />
                <button onClick={()=>saveData(instaData,'instagram',setInstagram)}>
                  저장
                </button></div> 
              : instaData ||'정보를 입력해 주세요.'}
        </div>
        <div className="wrap" onClick={onTwitter}><TwitterIcon style={{fontSize:25, color:'rgba(0,0,0,.8)'}}/>
            {twitter 
              ? <div className="box" onClick={onbreakEvent}>
                <Input onChange={(event)=>onChangeData(event,setTwitterData)} value={twitterData || ''} className="input" name={'주소를 입력해주세요.'} />
                <button onClick={()=>saveData(twitterData,'twitter',setTwitter)}>
                  저장
                </button></div> 
              : twitterData ||'정보를 입력해 주세요.'}
        </div>
        <div className="wrap" onClick={onHome}><HomeIcon style={{fontSize:25, color:'rgba(0,0,0,.8)'}}/>
            {home 
              ? <div className="box" onClick={onbreakEvent}>
                  <Input onChange={(event)=>onChangeData(event,setHomeData)} value={homeData || ''} className="input" name={'주소를 입력해주세요.'} />
                  <button onClick={()=>saveData(homeData,'home',setHome)}>
                    저장
                  </button>
                </div> 
              : homeData ||'정보를 입력해 주세요.'}
        </div>
      </div>
    </SocialBox>
  )
