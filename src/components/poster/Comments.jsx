...
const Comments = ({postId}) => {
  ...
  return (
    <div style={{maxWidth:'880px', margin:'0 auto', fontFamily:'Mapo'}}>
      <h3 id='commentView' style={{marginTop:30}}>
        {comments.length} 답변
      </h3>
      ...
      {comments[0] && comments.map(
        (res, i) => res.seq === 1 ? <ReplyBox id={res.id} key={res.id} path={res.profile_img}>
                    ...
                    <span className="replys" onClick={()=>onClickReply(i)}>
                      <Icon name="reply" /> 댓글 달기
                    </span>
                { reply && (i===reply -1 ) 
                  ? <div className="replyBox">
                      <form>
                        { userInfo 
                          ? <textarea onChange={onChangeChild} value={childValue} /> 
                          : <textarea readOnly placeholder='로그인이 필요합니다.' />
                        }
                        <Button onClick={(event) => onReplyChild(event,res.id)} width="70px" size='.85rem'>
                          댓글 작성
                        </Button>
                      </form>
                    </div> 
      ...
      <CommentContainer>
        ...
        <form>
          { userInfo 
            ? <textarea onChange={onChangeParent} value={parentValue}></textarea> 
            : <textarea readOnly placeholder='로그인이 필요합니다.' /> 
          }
          <Button onClick={onReplyParent}>
            댓글 작성
          </Button>
        </form>
      </CommentContainer>
