    ...
    return data.map((search) => {
      return (
        <FeedBox  key={search.id} url={search.tumnailImg} img={search.user.profile_img}>
          ...
          <div className='poster_title' onClick={()=>{history.push(`/poster/${search.id}/${null}`)}}>
            {search.tumnailTitle}
          </div>
          <div className='poster_preview'>
            { search.tumnailImg && (
              <div className="poster_tumnail">
                <img src={ImageEnv(search.tumnailImg)} />
              </div>
            ) }
            <div className='poster_content'>
              <p>
                { search.content.blocks.map((block) => {
                  switch (block.type) {
  ...
  return (
    ...
    <SearchBox >
      <div className="explanation">
        <p>
          KeyWord로 검색해보세요! 원하는 정보를 더 쉽고 빠르게 찾을 수 있습니다. 해당 KeyWord는 포스트의 제목 또는 내용에 매치됩니다.
        </p>
      </div>
      <Input className="searchInput" ref={inputFocus} name={'# 키워드를 입력해주세요'} onChange={onchangeValue} value={keyWord} />
      <Detail />
      <div>
        { posts && searchComponent(posts) }
      </div>
    </SearchBox>
    </>
  )
